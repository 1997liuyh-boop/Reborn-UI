...
<template>
    <RebornPage title="选择器" description="用于从一组选项中选择一个值的组件。">
        ...
        <RebornCard title="基础用法" custom-class="space-y-4">
            ...
            <view class="text-xs">插槽</view>
            <RebornSelect v-model="form.selected2" :options="options2" :disabled="isDisabled"
                :show-cancel="isShowCancel" :clearable="isShowClear" :size="currentSize" :color="currentColor"
                :confirm-text="isButtonText ? '下一步' : '确定'" :cancel-text="isButtonText ? '关闭' : '取消'">
                <template #tag="{ selectItem }">
                    <view v-for="value in selectItem" :key="value.value" class="flex items-center">
                        <view class="w-3 h-3 rounded-full mr-1" :style="{ backgroundColor: value.value }" />
                        <text :style="{ color: value.value }">{{ value.label }}</text>
                    </view>
                </template>
                <template #option="{ item, index }">
                    <view class="w-3 h-3 rounded-full mr-1" :style="{ backgroundColor: item.value }" />
                    <text :style="{ color: item.value }">{{ item.label }}</text>
                </template>
            </RebornSelect>

            <view class="text-xs">自定义样式</view>
        ...
        <RebornCard title="控制器" custom-class="space-y-4">
            ...
            <view class="flex justify-between">
                ...
                <view class="flex flex-wrap gap-2">
                    <view v-for="c in selectColors" :key="c"
        ...
        <RebornCard title="自定义触发器" custom-class="space-y-4">
            <view class="text-xs text-gray-4">通过 show-trigger 隐藏默认触发器，使用 ref 调用 open 方法打开</view>
