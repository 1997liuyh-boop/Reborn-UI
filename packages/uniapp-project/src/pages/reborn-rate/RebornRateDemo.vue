    ...
    <RebornPage title="评分" description="用于对事物进行评级操作的组件。">
        ...
        <RebornCard title="用法" custom-class="space-y-4">
            <view class="flex items-center justify-between">
                <text class="text-sm text-slate-600 dark:text-gray-5">基础</text>
            ...
            <view class="flex items-center justify-between">
                <text class="text-sm text-slate-600 dark:text-gray-5">爱心</text>
                <RebornRate v-model="val1" icon="i-lucide-heart" active-icon="i-lucide-heart" :size="currentSize"
                    :color="currentColor" :allow-half="isAllowHalf" :show-value="isShowValue" :disabled="isDisabled"
                    :readonly="isReadonly" :count="maxCount" />
            </view>
            <view class="flex items-center justify-between">
                <text class="text-sm text-slate-600 dark:text-gray-5">火焰</text>
                <RebornRate v-model="val1" icon="i-mdi-brightness-5" half-icon="i-mdi-brightness-6"
                    active-icon="i-mdi-brightness-4" :size="currentSize" :color="currentColor" :allow-half="isAllowHalf"
                    :show-value="isShowValue" :disabled="isDisabled" :readonly="isReadonly" :count="maxCount" />
            </view>
            <view class="flex items-center justify-between">
                <text class="text-sm text-slate-600 dark:text-gray-5">篮球</text>
                <RebornRate v-model="val1" icon="i-mdi-basketball" active-icon="i-mdi-basketball" :size="currentSize"
                    :color="currentColor" :allow-half="isAllowHalf" :show-value="isShowValue" :disabled="isDisabled"
                    :readonly="isReadonly" :count="maxCount" />
            </view>
            <view class="flex items-center justify-between">
                <text class="text-sm text-slate-600 dark:text-gray-5">图片 URL</text>
                <RebornRate v-model="val1" :size="currentSize" :color="currentColor" :allow-half="isAllowHalf"
                    :show-value="isShowValue" :disabled="isDisabled" :readonly="isReadonly" :count="maxCount">
                    <template #icon="{ active, style }">
                        <image src="https://www.leyifan.com/favicon.ico" class="w-full h-full"
