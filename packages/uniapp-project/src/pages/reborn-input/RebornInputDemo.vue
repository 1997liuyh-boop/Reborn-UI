...
<template>
    <RebornPage title="输入框" description="用于接收用户输入的数据。">
        ...
        <RebornCard title="全局尺寸控制" custom-class="flex flex-row gap-2">
            <ReButton v-for="size in sizes" :key="size" :size="size === 'lg' ? 'md' : 'sm'"
            ...
            <view class="space-y-2">
                ...
                <view class="flex flex-wrap gap-2">
                    <view v-for="t in inputTypes" :key="t">
                        <ReButton :color="currentType === t ? 'primary' : 'info'" @click="currentType = t" size="xs">
                            {{ t }}
                        </ReButton>
                    </view>
                </view>
            </view>
            <RebornInput v-model="input8" :size="currentSize" :type="currentType"
        ...
        <RebornCard title="插槽与图标" custom-class="flex flex-col gap-2">
            <RebornInput v-model="input5" :size="currentSize" placeholder="自定义前置图标">
                <template #leading>
                    <view class="i-lucide-search text-gray-400" :class="currentSize === 'sm' ? 'size-3.5' : 'size-4'" />
                </template>
            </RebornInput>

            <RebornInput v-model="input5" :size="currentSize" placeholder="自定义后置图标">
                <template #trailing>
                    <view class="i-lucide-calendar text-gray-400"
            ...
            <RebornInput v-model="input7" :size="currentSize" placeholder="自定义后置文本">
                <template #trailing>
                    <text class="text-gray-500 font-medium"
                        :class="currentSize === 'sm' ? 'text-xs' : 'text-sm'">RMB</text>
                </template>
            </RebornInput>
        </RebornCard>
