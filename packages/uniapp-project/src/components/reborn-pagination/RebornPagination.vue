...
/** 确认跳转输入：钳制到 [1, totalPages]，非法输入回退当前页 */
function handleJumperConfirm() {
  const value = Number(jumperValue.value)
...
function handleSizeChange(size: number | string) {
  const next = Number(size)
  pageSizeModel.value = next
...
const ui = computed(() => {
  ...
  return {
    ...
    input: (opts?: { class?: any }) =>
      b({ ...base(), active: false, disabled: props.disabled }).input({
...
<template>
  <view v-if="!hideOnSinglePage || totalPages > 1" :class="ui.root()">
    <!-- 简洁模式：只渲染 上一页 / 当前页-总页数 / 下一页 -->
    <template v-if="simple">
      <!-- 上一页 -->
      <slot name="prev" :disabled="prevDisabled" :prev="prev">
        <view :class="ui.prev({ disabled: prevDisabled })" @tap="prev">
      ...
      <slot name="next" :disabled="nextDisabled" :next="next">
        <view :class="ui.next({ disabled: nextDisabled })" @tap="next">
    ...
    <template v-else>
      <template v-for="token in layoutList" :key="token">
        ...
        <!-- 页码列表（含首尾与省略号） -->
        <view v-else-if="token === 'pager'" :class="ui.pager()">
          <template v-for="item in pagerList" :key="item.key">
            <!-- 省略号：点击向对应方向跳转 -->
            <view
              v-if="item.type === 'ellipsis'"
              :class="ui.ellipsis()"
              @tap="handleEllipsisJump(item.direction)"
            >
              <text>•••</text>
            </view>
            ...
            >
              <view :class="ui.pagerItem({ active: item.active, disabled: pagerDisabled })" @tap="setPage(item.page)">
                <text>{{ item.page }}</text>
              </view>
            </slot>
          </template>
        ...
        <!-- 跳转输入 -->
        <view v-else-if="token === 'jumper'" :class="ui.jumper()">
        ...
        <!-- 总数 -->
        <view v-else-if="token === 'total'" :class="ui.total()">
          <slot name="total" :total="props.total">
            <text>共 {{ props.total }} 条</text>
          </slot>
        </view>

        <!-- 每页条数选择 -->
        <view v-else-if="token === 'sizes'" :class="ui.sizes()">
          <slot
            name="sizes"
