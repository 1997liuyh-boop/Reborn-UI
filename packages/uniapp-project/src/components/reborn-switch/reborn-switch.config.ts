...
export default {
  slots: {
    ...
    activeTrack: "",
    inactiveTrack: "bg-gray-3",
    input: "sr-only",
    track:
      "relative inline-flex items-center rounded-full transition-colors ring-1 ring-transparent",
    ...
    activeLabel: "text-28 text-gray-8 dark:text-gray-1",
    inactiveLabel: "text-28 text-gray-8 dark:text-gray-1",
  },
  variants: {
    active: {
      true: {
        ...
        thumb: "left-[max(2px,calc(100%_-_var(--re-switch-thumb-size)_-_2px))] right-[2px] motion-safe:group-[.re-switch-pressed:not([data-disabled=true])]/switch:left-[max(2px,calc(100%_-_var(--re-switch-thumb-size)*1.3_-_2px))]",
      ...
      false: {
        ...
        thumb: "motion-safe:group-[.re-switch-pressed:not([data-disabled=true])]/switch:right-[max(2px,calc(100%_-_var(--re-switch-thumb-size)*1.3_-_2px))]",
        activeLabel: "text-gray-4 dark:text-gray-6",
        inactiveLabel: "text-gray-9 dark:text-gray-1 font-medium",
      },
    },
    /**
     * 尺寸规格（与 web 端同一套）：滑块四周留白固定 2px（基础槽的 left-[2px] / top-[2px]），
     * 选中位移 = 轨道宽 − 滑块直径 − 4px 留白，见 compoundVariants。
     */
    ...
    color: {
      primary: { activeTrack: "bg-primary" },
      secondary: { activeTrack: "bg-secondary" },
      success: { activeTrack: "bg-success" },
      info: { activeTrack: "bg-info" },
      warning: { activeTrack: "bg-warning" },
      error: { activeTrack: "bg-error" },
      neutral: { activeTrack: "bg-neutral" },
    },
    /**
