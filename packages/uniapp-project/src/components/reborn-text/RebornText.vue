<template>
    <!-- #ifdef MP -->
    <view :class="ui.base({ class: ptClassName })" :style="textStyle" :selectable="selectable" :space="space"
        :decode="decode" :key="cache.key">
        <slot>{{ content }}</slot>
    </view>
    <!-- #endif -->

    <!-- #ifndef MP -->
    <text :class="ui.base({ class: ptClassName })" :style="textStyle" :selectable="selectable" :space="space"
        :decode="decode" :key="cache.key">
        <slot>{{ content }}</slot>
    </text>
    <!-- #endif -->
...
// Helper shims for missing dependencies
const cache = { key: 1 };
type PassThrough = { className?: string };
