...
<template>
  <div class="flex w-full min-w-0 flex-col">
    ...
    >
      <RebornAlert
        :key="`${state.closable}`"
        :type="state.type" :variant="state.variant" :title="state.title || undefined"
        :show-icon="state.showIcon" :closable="state.closable" :banner="state.banner" :center="state.center"
      >
        {{ state.content }}
      </RebornAlert>
    </Playground>
    ...
    <DemoSection
      title="基本用法"
      description="五种消息类型对应五种语义配色与默认图标；normal 为 2.41.0 新增类型，用于公告等中性场景。"
    >
    ...
    <DemoSection
      title="视觉变体"
      description="六种视觉变体对齐按钮组件的同名变体（不含 circle），默认为 soft 浅底。"
    >
      <DemoBlock layout="stack">
    ...
    <DemoSection
      title="标题与操作项"
      description="通过 title 属性（或 title 插槽）设置标题，action 插槽放置右侧操作项。"
    >
      <DemoBlock layout="stack">
    ...
    <DemoSection
      title="可关闭"
      description="closable 展示关闭按钮，点击触发 close 事件；关闭动画结束后触发 after-close，支持 v-model:show 受控。"
    >
      <DemoBlock layout="stack">
        ...
        <div class="flex items-center gap-3">
          <RebornButton v-if="!closableShow" size="sm" @click="closableShow = true; afterCloseLog = ''">重新显示</RebornButton>
    ...
    <DemoSection
      title="顶部公告（banner）"
      description="banner 模式去除边框和圆角，适合置于页面顶部；可配合 center 让内容居中。"
    >
      <DemoBlock layout="stack">
    ...
    <DemoSection
      title="消息轮播通知栏"
      description="通过 messages 传入多条消息即变为轮播通知栏，垂直轮播展示，interval 控制间隔，悬停暂停轮播。"
    >
      <DemoBlock layout="stack">
        <RebornAlert
        ...
        />
        <span class="text-sm text-gray-6">当前第 {{ noticeIndex + 1 }} / {{ noticeMessages.length }} 条</span>
      </DemoBlock>
    </DemoSection>
  </div>
</template>
