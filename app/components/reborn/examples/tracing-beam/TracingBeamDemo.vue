    ...
    <TracingBeam class="px-6">
      <div class="relative mx-auto max-w-2xl pt-4 antialiased">
        <div v-for="(item, index) in dummyContent" :key="`content-${index}`" class="mb-10">
          <div class="mb-4 w-fit rounded-full bg-black px-2 text-sm text-white dark:bg-white dark:text-black">
            {{ item.badge }}
          </div>
          ...
          <div class="prose prose-sm dark:prose-invert text-sm">
            <img v-if="item.image" :src="item.image" alt="blog thumbnail" class="mb-10 rounded-lg object-cover" />
            <div>
