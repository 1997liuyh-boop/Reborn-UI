...
import { menuModes, menuTriggers, menuColors } from "~/components/reborn/ui/reborn-menu/reborn-menu.config";
...
const controls: any = [
  ...
  },
  {
    ...
    children: [
      ...
      { label: "单一展开", key: "uniqueOpened", component: "checkbox" as const, defaultValue: false },
    ],
  },
];
...
<template>
  <div class="flex flex-col gap-16 pb-24">
    <!-- 第一部分：交互式游乐场 -->
    <Playground v-model="state" :controls="controls" :code="`<RebornMenu
  v-model:active='${JSON.stringify(activePath)}'
  :mode='${state.mode}'
...
/>`" component-name="RebornMenu" title="交互体验" description="通过左侧面板实时调节组件属性，在右侧查看视觉反馈">
      <RebornMenu v-model:active="activePath" :mode="state.mode" :collapse="state.collapse"
        :menu-trigger="state.menuTrigger" :unique-opened="state.uniqueOpened" :color="state.color" class="min-h-[400px]"
