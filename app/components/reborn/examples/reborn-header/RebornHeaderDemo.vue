...
<template>
  <div class="flex w-full min-w-0 flex-col">
    <Playground
      ...
      :controls="controls"
      :code="headerCode"
      component-name="RebornHeader"
      title="交互演练场"
      description="在下方可滚动的宿主区域内滚动内容，即可观察吸顶行为；窄屏时右上角会出现侧边栏开关。"
    >
      <!-- 宿主区域：仅描边不填充，模拟一个独立的滚动视口 -->
      <div
        class="border-default rounded-ui-sm relative h-[420px] w-full overflow-y-auto border"
      >
        <RebornHeader
          :title="state.title"
          to="/"
          :toggle-side="state.toggleSide"
          :sticky="state.sticky"
        >
          <!-- 中间导航：默认插槽 -->
          <nav class="flex items-center gap-8">
            ...
            >
              ...
              <Icon
                ...
                class="ml-1 inline size-3 opacity-50"
              />
            </NuxtLink>
          </nav>
        ...
        <div class="flex flex-col gap-4 p-6">
          ...
          <div
            v-for="i in 6"
            :key="i"
            class="border-default rounded-ui-sm h-32 shrink-0 border border-dashed"
          />
        </div>
      </div>
    </Playground>
    ...
    <DemoSection
      title="插槽结构"
      description="顶栏分为左、中、右三个区域，默认插槽填充中间导航，#right 承载操作按钮，#content 则是窄屏侧边栏的内容。"
    >
