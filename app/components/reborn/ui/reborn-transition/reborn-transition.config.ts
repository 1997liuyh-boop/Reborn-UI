...
export const transitionStyles: Record<string, Record<string, string>> = {
  ...
  // 注意：Tailwind v4 的 translate-* / scale-* 输出的是独立的 translate / scale CSS 属性（非 transform），
  // 过渡属性必须写 translate / scale，写 transform 会导致位移与缩放瞬跳、只剩透明度在动
  'fade-up': {
    // 从上淡入淡出 (starts from top)
    enter: '-translate-y-8 opacity-0',
    'enter-active': `transition-[opacity,translate] ${EASE_ENTER}`,
    'enter-to': 'translate-y-0 opacity-100',
    leave: 'translate-y-0 opacity-100',
    'leave-active': `transition-[opacity,translate] ${EASE_LEAVE}`,
  ...
  'slide-up': {
    ...
    'enter-active': `transition-[translate] ${EASE_ENTER}`,
    'enter-to': 'translate-y-0',
    leave: 'translate-y-0',
    'leave-active': `transition-[translate] ${EASE_LEAVE}`,
  ...
  'zoom-in': {
    // 缩入 (从 0.95 放大到 1，关闭时缩小到 0.92)；scale-* 是独立 scale 属性，过渡必须写 scale
    enter: 'opacity-0 scale-95',
    'enter-active': `transition-[opacity,scale] duration-300 [transition-timing-function:cubic-bezier(0.34,1.56,0.64,1)]`,
    'enter-to': 'opacity-100 scale-100',
    leave: 'opacity-100 scale-100',
    'leave-active': `transition-[opacity,scale] duration-250 [transition-timing-function:cubic-bezier(0.4,0,0.2,1)]`,
