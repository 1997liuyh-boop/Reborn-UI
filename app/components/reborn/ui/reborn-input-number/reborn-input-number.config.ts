...
export default {
  slots: {
    wrapper:
      "relative inline-flex items-center overflow-hidden rounded-full bg-gray-2 text-gray-8 ring-1 ring-transparent transition-colors focus-within:bg-white focus-within:ring-2 data-[disabled=true]:bg-gray-3 data-[disabled=true]:text-gray-5",
    button:
      "flex h-full items-center justify-center px-4 text-gray-6 transition-colors disabled:cursor-not-allowed disabled:text-gray-4",
    input:
      "min-w-0 flex-1 bg-transparent text-center text-gray-8 outline-none placeholder:text-gray-4 [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none",
    divider: "h-1/2 w-px bg-gray-3",
  },
  variants: {
    size: {
      sm: {
        wrapper: "h-[var(--input-sm-height)] text-[length:var(--text-size-26)]",
        input: "text-[length:var(--text-size-26)]",
      },
      md: {
        wrapper: "h-[var(--input-md-height)] text-[length:var(--text-size-28)]",
        input: "text-[length:var(--text-size-28)]",
      },
      lg: {
        wrapper: "h-[var(--input-lg-height)] text-[length:var(--text-size-32)]",
