...
function updateCircleLayout() {
    ...
    const angleStep = totalAngle === 360
        ? totalAngle / count
        : (count > 1 ? totalAngle / (count - 1) : 0)
...
watch([isActive, fabDirection, () => props.radius, () => props.startAngle, () => props.totalAngle], () => {
    updateCircleLayout()
}, { immediate: true })
...
function handlePointerUp(e: PointerEvent) {
    ...
    const currentTopAfter = typeof topPos.value === 'number' ? topPos.value : parseFloat(String(topPos.value))
    const currentLeftAfter = typeof leftPos.value === 'number' ? leftPos.value : parseFloat(String(leftPos.value))
...
const rootStyle = computed<CSSProperties>(() => {
    const style: CSSProperties = {
        ...
        zIndex: props.zIndex,
        // 如果处于吸边动画中，应用平滑位移过渡
        ...(attractTransition.value ? { transition: 'top 0.3s cubic-bezier(0.18, 0.89, 0.32, 1.28), left 0.3s cubic-bezier(0.18, 0.89, 0.32, 1.28)' } : {}),
