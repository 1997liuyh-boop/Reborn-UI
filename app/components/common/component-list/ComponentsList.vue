...
const categories = computed(() => {
  ...
  return ["all", "new", "updated", ...uniqueCategories].map((name, index) => ({
    name,
    activeEmoji: activeEmojis[index % activeEmojis.length],
    inactiveEmoji: inactiveEmojis[index % inactiveEmojis.length],
  }));
...
const filteredComponents = computed(() => {
  ...
  if (selectedCategory.value === "new") {
    ...
    filtered = filtered.filter(component =>
      component.badge?.toLowerCase() === "updated" || component.badge?.toLowerCase() === "update",
    );
  }
    ...
    <div
      ...
      <RebornRadioGroup v-model="selectedCategory" size="lg">
        <RebornRadio v-for="category in categories" :key="category.name" :label="category.name" :value="category.name"
          :ui="{ root: 'px-2 rounded-full shadow-lg' }">
          <template #active-icon>
            <div
              class="animate-zoom relative flex h-8 w-8 items-center justify-center overflow-hidden rounded-lg border border-white/40 bg-linear-to-br from-white/40 to-white/10 shadow-xl backdrop-blur-lg transition-all duration-500 dark:border-white/20 dark:from-white/10 dark:to-transparent">
              <div class="absolute inset-0 bg-red-500/10 mix-blend-overlay" />
          ...
          <template #inactive-icon>
            <div
              class="flex items-center justify-center rounded-xl border border-white/10 bg-white/5 p-1.5 opacity-80 backdrop-blur-sm transition-all duration-300">
