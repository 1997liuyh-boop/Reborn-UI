// 复用用户提供的火焰算法，仅渲染装饰层，不依赖 HTML 截图或纹理。
export const flameShader = `
precision highp float;
uniform vec2 iResolution;
uniform float iTime;

// Simplex 噪声版权归 2011 Ashima Arts 所有，按 MIT 许可证分发。
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // 柔化画布上沿与两侧，避免叠加在视频上时出现矩形边界。
  float edgeFade = (1.0 - smoothstep(0.68, 1.0, uv.y))
    * smoothstep(0.0, 0.12, uv.x) * (1.0 - smoothstep(0.96, 1.0, uv.x));
  fragColor = vec4(clamp(effect, 0.0, 1.0), effectLevel * edgeFade);
}
