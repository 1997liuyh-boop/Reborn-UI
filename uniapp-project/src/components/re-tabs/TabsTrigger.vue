...
<template>
    <view role="tab" :id="triggerId" :aria-selected="isActive"
        ...
        :data-index="localIndex" @tap="handleClick">
        <view class="rb-tabs__trigger-inner inline-flex items-center justify-center gap-2">
            ...
            <text v-if="$slots['leading-avatar']"
            ...
            <text data-tab-label :class="context.ui.value.label({ class: context.uiOverrides.value?.label })">
                <slot name="label">
                    <slot></slot>
                </slot>
            </text>
            <text v-if="$slots['trailing-badge']"
