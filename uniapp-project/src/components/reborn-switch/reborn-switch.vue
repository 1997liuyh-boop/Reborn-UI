...
const ui = computed(() => {
  ...
  return {
    ...
    track: (opts?: { class?: any }) => styles.track({ class: cn(opts?.class) }),
    thumb: (opts?: { class?: any }) => styles.thumb({ class: cn(opts?.class) }),
...
<template>
  <view :class="ui.wrapper({ class: props.class })" @tap="onTap" :data-disabled="props.disabled || props.loading">
