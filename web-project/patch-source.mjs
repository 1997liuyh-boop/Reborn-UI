...
import { tryUseNuxt, resolveAlias, useLogger, updateTemplates, isIgnored, useNuxt, resolveModule, extendViteConfig, addTemplate, hasNuxtModule, addPlugin, installModule, addComponent, addServerHandler, addPrerenderRoutes, defineNuxtModule, createResolver, addImports, addServerImports, addTypeTemplate, addVitePlugin } from '@nuxt/kit';
...
async function isPackageInstalled(packageName) {
  try {
    ...
    console.error(`[Nuxt Content Patch] Failed to import ${packageName}:`, e);
    return false;
  }
}
  ...
  resolvedSource.prepare = async ({ rootDir }) => {
    ...
    if (repository) {
      ...
      if (resolvedSource.authBasic) {
        ...
        const encodedCredentials = typeof Buffer !== "undefined" ? Buffer.from(credentials).toString("base64") : btoa(credentials);
        headers = {
...
const nuxthub = definePreset({
  ...
  async setupNitro(nitroConfig, options) {
    ...
    const nuxthubVersion = nuxt.options.hub?.database === true ? 0.9 : 0.1;
