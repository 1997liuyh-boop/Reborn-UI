<template>
  <header class="bg-background/80 sticky top-0 z-40 backdrop-blur-lg" :class="{ 'border-b': config.header.border }"
    v-if="baseRouteName !== 'index'">
    <div class="flex h-14 items-center justify-between gap-2 px-4 md:px-8" :class="{
      ...
      <div class="flex flex-1 justify-end gap-2">
        ...
        <div class="flex">
          ...
          <NuxtLinkLocale v-for="(link, i) in config.header.links" :key="i" :to="localePath(link?.to)"
            :target="link?.target">
    ...
    <div v-if="baseRouteName !== 'index' && config.aside.levelStyle === 'header'" class="md:mt-2 md:px-8" :class="{
