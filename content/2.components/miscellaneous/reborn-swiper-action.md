...
| `mode`                | `'push' \| 'overlay'`         | `'push'`    | 展开模式：`push` 为内容平移推开操作区（传统）；`overlay` 为操作区从右侧叠加滑入，内容不移动             |
| `actionRadius`        | `'none' \| 'right' \| 'both'` | `'right'`   | 操作区外侧圆角策略：`none` 不补圆角；`right` 仅最右侧按钮补右圆角；`both` 最左与最右各补一侧圆角        |
| `touchHint`           | `boolean`                     | `false`     | 落指时给用户可滑动的视觉提示：`push` 模式对内容施加微小负偏移露出色条边缘；`overlay` 模式显示操作区边缘 |
...
### `SwiperActionItem`（`rightActions` 元素）
...
| 绑定      | 类型            | 默认值 | 描述                                                   |
| :-------- | :-------------- | :----- | :----------------------------------------------------- |
| `v-model` | `'right' \| ''` | `''`   | 当前展开侧；空字符串表示收起。变更时与同组其它实例互斥 |

## Emits

| 事件名          | 回调参数                   | 描述                                                                                                                                                  |
| :-------------- | :------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `change`        | `side: 'right' \| ''`      | 展开侧变化（与 `v-model` 同步）                                                                                                                       |
| `open`          | `side: 'right'`            | 从收起到展开                                                                                                                                          |
| `close`         | —                          | 从展开到收起                                                                                                                                          |
...
| 名称    | 类型                       | 描述                                |
| :------ | :------------------------- | :---------------------------------- |
| `open`  | `(side?: 'right') => void` | 展开右侧操作区；`disabled` 时不生效 |
| `close` | `() => void`               | 收起当前项                          |
...
| 插槽名         | 作用域参数                                       | 描述                                                                                             |
| :------------- | :----------------------------------------------- | :----------------------------------------------------------------------------------------------- |
| `default`      | `{ openSide: 'right' \| '', close: () => void }` | 滑动手势覆盖的主内容区                                                                           |
| `right-action` | `{ item, index, side: 'right', ui }`             | 自定义右侧单个操作格；`ui` 与组件内部 computed 一致（含 `root`、`content`、`action` 等类名工厂） |
...
- **展开模式**：`mode="push"`（默认）内容平移推开操作区；`mode="overlay"` 操作区从右侧叠加滑入，多按钮从左端展开舒展，内容不移动，适合不希望主体偏移的场景。
- **触摸提示**：`touchHint` 开启后落指瞬间给予可滑动的视觉反馈；`push` 模式内容微移露出底色条边缘，`overlay` 模式操作区边缘短暂出血。
- **圆角策略**：`actionRadius` 控制操作按钮的外侧圆角；`'right'`（默认）仅最右侧按钮补右圆角，与根容器圆角对齐；`'both'` 同时给最左侧按钮补左圆角；`'none'` 全部直角。
...
- 组件从 `.vue` 中导出 `SwiperActionItem`、`SwiperActionClickPayload` 等类型，便于 TypeScript 项目书写 `rightActions` / 事件处理函数。
