...
| `menuTrigger`          | `'hover' \| 'click'`                                                                   | `'hover'`                  | 子菜单的触发方式。只对浮层展开生效；平铺展开（`expandType="normal"`）一律按 `click` 处理                                                                                                                       |
...
| `tooltip`              | `boolean \| MenuTooltipConfig`                                                         | `true`                     | 折叠态下一级菜单项悬停时的文字提示。`false` 关闭（回落为原生 `title`）；传对象则透传给 `RebornTooltip`，见「折叠态的文字提示」                                                                                 |
| `autoScrollIntoView`   | `boolean`                                                                              | `false`                    | 选中项变化及首次挂载时，是否把选中项滚动到可见区域。选中项藏在未展开的子菜单里时，改为滚到被高亮的祖先子菜单标题                                                                                               |
| `scrollConfig`         | `ScrollIntoViewOptions`                                                                | -                          | 自动滚动的参数，`block` / `inline` / `behavior` 同原生 `scrollIntoView`，默认 `{ block: 'nearest', inline: 'nearest' }`；`behavior` 缺省时挂载即时定位、之后平滑滚动；只滚最近的滚动容器，不带动页面           |
...
- **折叠态一级菜单项的原生 `title` 改为 `RebornTooltip` 提示**，默认开启。原生 `title` 要悬停约 1 秒才出现、样式不可控，且没传 `title` 时什么也不显示；新提示缺省取菜单项标题。需要旧行为传 `:tooltip="false"`。
...
- **平铺展开（`expandType="normal"`）不再响应悬停，一律按点击展开与收起**，`menuTrigger` 只对浮层展开生效（见「子菜单的展开与收起语义」）。此前默认的 `hover` 触发下，悬停满 `showTimeout` 子菜单就已展开，紧接着的点击又把它收回，表现为要点两三次才展开；悬停展开还会让下方条目在指针底下跳动。依赖平铺态悬停展开的话，请改用 `expandType="popup"`。
...
- **浮层展开 + `menuTrigger="hover"` 下选中后会把展开集合裁剪到选中项的祖先链**（见「选中菜单项后的收起规则」），与悬停只维持一条展开路径的口径一致。点击触发（含全部平铺展开）不裁剪，那里允许多条分支同时展开。
...
- **平铺子菜单容器去掉了圆角，展开落定后不再裁剪溢出**。此前容器沿用浮层的 `rounded-md` 且常驻 `overflow-hidden`，首个子条目的选中背景被裁出上圆角，选中投影也被切掉上半截、只剩底边一道阴影。裁剪现在只在收起与高度动画期间保留。
...
- **浮层展开（`expandType="popup"`）现在无条件只保留一条展开路径**（见「子菜单的展开与收起语义」）。此前只有悬停触发与 `uniqueOpened` 会裁剪路径，浮层 + 点击触发走的是累加分支：点开「系统管理」再点同级的「数据分析」，两块浮层会同时悬在页面上互相遮挡。平铺展开不受影响，仍可多条分支并存。若原先依赖浮层多开，请改用 `expandType="normal"`。
